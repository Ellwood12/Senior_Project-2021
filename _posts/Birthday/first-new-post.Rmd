...
description: |
  Are birthday's all random or is there a good chance that someone in the has the same birthday?  As we unravel this paradox, you will be surprised to find that the chance of sharing the same special day, really isn't that surprising.
...
The analysis of this problem will require us to tackle the issue from an opposing view.  Instead focusing on the Probability of Sharing the same date, we are going to begin with the Probability of *Not* Sharing the date.
  The reason we will want t0 do that is because of the following:
...
Let's tackle this problem with an experiment by creating a classroom that is going to slowing fill up with students.  As the first students comes into the room, the probability that he doesn't share a birthday with any one in the room is 100%.  This should make sense because there is no one else in the room, so of course there is a 100% chance.
...
The real trouble will begin as the next student enters the room. Now, we are trying to calculate the chance that the next person WILL NOT have the same birthday; the way to do that will be in two steps.
...
From this point, the previous steps are applied for as many people are present to calculate the probability of Not Sharing a Birthday.  To find the probability of Sharing the same birthday will require one simple addition.  
...
The plot below helps us visualize how the probability of sharing a birthday changes according to the group size (number of people).  The code to creating this plot will be placed below.  It is fasinating to see how the percentages change.
